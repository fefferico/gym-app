  ...
  private async startRestPeriod(duration: number, completedExIndex: number, completedSetIndex: number): Promise<void> {
    ...
    if (exercise.supersetId) {
      ...
      if (historicalSet) {
        ...
        if (parts.length > 0) {
          detailsLine = `${this.translate.instant('restTimer.lastTime')}: ${parts.join(' | ')}`;
        }
        ...
        if (repsDisplay && repsDisplay !== '0' && weightDisplay !== this.translate.instant('workoutService.display.weightNotApplicable')) {
          parts.push(`${weightDisplay} x ${repsDisplay} reps`);
        }
        ...
        if (parts.length > 0) {
          detailsLine = `Target: ${parts.join(' | ')}`;
        } else {
          detailsLine = 'No target set';
        }
      }
  ...
  toggleSetCompletion(exercise: WorkoutExercise, set: ExerciseTargetSetParams, exIndex: number, setIndex: number, roundIndex: number, triggerAnimation: boolean = true): void {
    ...
    if (wasCompleted) {
      ...
      const newLoggedSet: LoggedSet = {
        ...
        targetRest: originalSet?.targetRest ?? targetSetValues?.targetRest ?? restToExact(0),
        targetReps: originalSet?.targetReps || targetSetValues?.targetReps || genRepsTypeFromRepsNumber(0),
        targetWeight: originalSet?.targetWeight || targetSetValues?.targetWeight || weightToExact(0),
        targetDuration: originalSet?.targetDuration || targetSetValues?.targetDuration || durationToExact(0),
        targetDistance: originalSet?.targetDistance || targetSetValues?.targetDistance || distanceToExact(0),
  ...
  private _prefillPerformanceInputs(): void {
    ...
    routine.exercises.forEach((exercise, exIndex) => {
      exercise.sets.forEach((plannedSet, setIndex) => {
        ...
        // Reps
        setValues.actualReps = plannedSet.targetReps;
        ...
        setValues.tempoLogged = plannedSet.targetTempo ?? undefined;
